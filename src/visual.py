...
def _gradiente_diagonal(largura: int, altura: int, cor_inicio: str, cor_fim: str) -> Image.Image:
    # gera numa grade pequena e amplia com interpolação bilinear —
    # resultado idêntico ao cálculo pixel a pixel, sem depender de numpy
    ri, gi, bi = _hex_para_rgb(cor_inicio)
    rf, gf, bf = _hex_para_rgb(cor_fim)
    pw, ph = 64, 32
    pixels = []
    for y in range(ph):
        for x in range(pw):
    ...
    img.putdata(pixels)
    return img.resize((largura, altura), Image.BILINEAR)
